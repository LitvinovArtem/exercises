...
lastDigit n = if ((abs n) < 10) then abs n else lastDigit (mod (abs n) 10)
...
minmax x y z = maxNumber - minNumber
  where
    maxNumber = max (max x y) z
    minNumber = min (min x y) z 
...
strSum str = sum nums
  where
    wordsList = words str
    nums = map closure wordsList
    closure = (read::String -> Integer)
...
lowerAndGreater n list = show n ++ " is greater than " ++ show lesserCount ++ " elements and lower than " ++ show biggerCount ++ " elements"
  where
    ...
    lesserNumbers = filter (<n) list
    biggerNumbers = filter (>n) list
